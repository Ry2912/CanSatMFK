...
int lat_dd; // 緯度の10進部分
int lon_ddd; // 軽度の10進部分
int lat_mm; // 緯度の小数点前の分数
int lat_mmmm; // 緯度の小数点後の分数
int lon_mm; // 経度の小数点前の分数
int lon_mmmm; // 軽度の小数点後の分数

int lat_mmmmmm; // 緯度の分数部分をまとめたもの
int lon_mmmmmm; // 軽度の分数部分をまとめたもの

int lat_dddddd; // 10進表記の緯度
int lon_dddddd; // 10進表記の軽度
    ...
    if (gps_cnt == 0) // 並列処理
    {
        for (int k = 0; k < 10; k++)
        {
            ...
            if (line != "")
            {
                ...
                // StringListの生成（空リスト、ここに$GPRMCを入れる）
                String list[30];
                for (i = 0; i < 30; i++)
                {
                    list[i] = "";
                ...
                if (list[0] == "$GPRMC")
                {
                    ...
                    {
                        lat_dd = (list[3].substring(0, 2)).toInt(); // 緯度の10進部分（度数）取り出し
                        lon_ddd = (list[5].substring(0, 3)).toInt(); // 軽度の10進部分（度数）取り出し

                        lat_mm = (list[3].substring(2, 4)).toInt(); // 緯度の小数点前の分数
                        lat_mmmm = (list[3].substring(5, 9)).toInt(); // 緯度の小数点後の分数
                        lon_mm = (list[5].substring(3, 5)).toInt(); // 軽度の小数点前の分数
                        lon_mmmm = (list[5].substring(6, 10)).toInt(); // 軽度の小数点後の分数

                        
                        lat_mmmmmm = lat_mm * 10000 + lat_mmmm; // 緯度の分数をまとめる
                        lon_mmmmmm = lon_mm * 10000 + lon_mmmm; // 軽度の分数をまとめる

                        lat_dddddd = lat_mmmmmm / 0.6; // 緯度の分数を10進表記に
                        lon_dddddd = lon_mmmmmm / 0.6; // 軽度の分数を10進表記に

                        now_lat = ((float)lat_dd) + ((float)lat_mmmmmm / 600000); // 10進の緯度
                        now_lon = ((float)lon_ddd) + ((float)lon_mmmmmm / 600000); // 10進の軽度
    ...
    if (mode != STOP_MODE)
    {
        if (twe_cnt == 0) // 並列処理
        {
            //if (TWE_Serial.available()) // これがあると一生読み込まない…
            {
                ...
                TWE_Serial.print(":0201"); // PCのTWELITEアドレスを02に設定
